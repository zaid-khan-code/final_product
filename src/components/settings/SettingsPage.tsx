...
export default function SettingsPage({ title, columns, data, modalFields, onAdd, onEdit, onDelete }: SettingsPageProps) {
  ...
  const openEdit = (idx: number) => {
    ...
    fields.forEach((f, fi) => {
      vals[f.label] = String(row[f.label.toLowerCase()] !== undefined ? row[f.label.toLowerCase()] : (keys[fi] !== undefined ? row[keys[fi]] : ''));
    });
    ...
    <div>
      ...
      <div className="card">
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {data.length === 0 ? (
                <tr><td colSpan={columns.length + 1} style={{ textAlign: 'center', padding: 40, color: 'var(--t3)', fontSize: 13 }}>No items yet. Click Add to create one.</td></tr>
              ) : (
                data.map((row, i) => (
                  <tr key={i}>
                    {columns.map((_, j) => {
      ...
      <Modal open={addModal} onClose={() => setAddModal(false)} title={`Add ${title.replace(/s$/, '')}`} footer={
        ...
        {fields.map((f, i) => (
          <div className="form-group" key={i}>
            ...
            {f.options ? <select className="input select-input" onChange={e => formRef.current[f.label] = e.target.value}><option value="">Select...</option>{f.options.map((o: string) => <option key={o} value={o}>{o}</option>)}</select> :
      ...
      <Modal open={editIdx !== null} onClose={() => setEditIdx(null)} title={`Edit ${title.replace(/s$/, '')}`} footer={
        ...
        {fields.map((f, i) => (
          <div className="form-group" key={i}>
            <label className="form-label">{f.label}</label>
            {f.options ? <select className="input select-input" defaultValue={formRef.current[f.label] || ''} onChange={e => formRef.current[f.label] = e.target.value}>{f.options.map((o: string) => <option key={o} value={o}>{o}</option>)}</select> :
