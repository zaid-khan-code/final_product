...
export default function MyAttendancePage() {
  ...
  const onAck = async () => {
    ...
    setAcking(true)
    const res = await apiFetch<DailyRow>(`/attendance/${row.attendance_id}/ack`, { method: 'PATCH', body: {} })
    setAcking(false)
    if (!res.ok) {
      showToast(res.error, 'error')
      return
  ...
  return (
    <div className="section-grid">
      ...
      <ComingSoonCard
        title="Attendance disputes and penalty review"
        description="Attendance dispute workflows and automated penalty review are not part of the current implementation phase."
      />
    </div>
  )
}
