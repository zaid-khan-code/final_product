      ...
      <div className="card" style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <div className="input-wrap" style={{ flex: 1, minWidth: 200 }}>
            <Search size={13} />
            <input className="input" placeholder="Search by name, designation, ID..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(230px, 1fr))', gap: 10 }}>
                {emps.map((e: any) => (
                  <div key={e.id} className="card" style={{ padding: '14px 16px', display: 'flex', gap: 12, alignItems: 'flex-start', transition: 'box-shadow .15s', cursor: 'default' }}
                    onMouseEnter={el => (el.currentTarget.style.boxShadow = '0 4px 20px rgba(0,0,0,.1)')}
